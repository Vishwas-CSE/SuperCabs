...
class CabDriver():
    ...
    def reward_func(self, state, action, Time_matrix):
        ...
        if pickup == 0 and drop == 0:
            ...
            time_taken_before_pickup = Time_matrix[start_loc, pickup, int(time), int(day)]
            ride_time = (time +  time_taken_before_pickup) % t
            ride_days = (day + (time +  time_taken_before_pickup)//t) % d

            reward = (R*Time_matrix[pickup, drop, int(ride_time),int(ride_days)] - C*(Time_matrix[pickup, drop, int(ride_time), int(ride_days)] + Time_matrix[start_loc, pickup, int(time), int(day)]))
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        if pickup == 0 and drop == 0:
            ...
            time_taken_before_pickup = Time_matrix[start_loc, pickup, int(time), int(day)]
            time_next_temp = (time +  time_taken_before_pickup) % t
            day_next_temp = (day + (time +  time_taken_before_pickup)//t) % d
            ...
            self.total_time = self.total_time +  time_elapsed + time_taken_before_pickup
